...
#include "VignettingCorrection/VC_LLF.hpp"
...
static float calH(float a,float b,float c,Mat GrayImg)
{
...
Runnable::RES_t VC_LLF::run(void)
{
	String path = "../data/VignettingCorrection/input.bmp";
	Mat img = imread(path,IMREAD_UNCHANGED);

	cvtColor(img,img,COLOR_BGR2GRAY);

	imwrite("../output/VignettingCorrection/original.bmp", img);
	...
	while(delta > limit)
	{
		cout << "delta = " << setprecision(6) << delta << ", limit = " << limit << endl;
	...
	imwrite("../output/VignettingCorrection/corrected.bmp", result);

	namedWindow("Corrected image", cv::WINDOW_NORMAL);
	imshow("Corrected image",result);
	waitKey(0);
	return Runnable::OK;
}
