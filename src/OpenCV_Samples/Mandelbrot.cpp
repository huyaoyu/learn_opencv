...
Runnable::RES_t Run_OCV_Mandelbrot::run(void)
{
	this->show_header();
...
Runnable::RES_t Run_OCV_Mandelbrot::runSample(void)
{
	...
	parallel_for_(Range(0, mandelbrotImg.rows*mandelbrotImg.cols), parallelMandelbrot, 4);
	//! [mandelbrot-parallel-call]
	...
	imwrite("../output/Mandelbrot_parallel.png", mandelbrotImg);
	imwrite("../output/Mandelbrot_sequential.png", mandelbrotImgSequential);
